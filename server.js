...
const viewEmployees = () => {
  connection.query(
    'SELECT employee.id, name, title, salary,first_name, last_name, manager_id FROM ((department JOIN role ON department.id = role.department_id) JOIN employee ON role.id = employee.role_id);',
    function (err, res) {
      if (err) throw err;
      console.table(res);
      mainMenu();
